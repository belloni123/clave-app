...
describe('read-only calendar access', () => {
  it.each([['POST', POST], ['PUT', PUT], ['DELETE', DELETE]] as const)('blocks %s before any Google Calendar side effect', async (method, handler) => {
    const request = new NextRequest('https://useclave.com.br/api/calendar/events', { method, body: JSON.stringify({ id: 'event', project_id: 'project', title: 'Test', date: '2026-09-10', type: 'test' }) })
